...
if ($failures.Count -eq 0) {
    ...
    Require-Text $flash 'FLASH_MODE=%~1|%~1.*prepare|prepare' 'Flash entry supports prepare mode'
    Require-Text $flash 'prepare' 'Flash entry documents prepare mode'
    Require-Text $flash 'run' 'Flash entry documents run mode'
    Require-Text $cycle 'flash_app\.bat"? run|flash_app\.bat.*run' 'Normal cycle explicitly uses run mode'
    Require-Text $faultBatText 'trigger' 'Fault entry supports reset-before-trigger mode'
    Require-Text $faultPs 'Trigger|trigger' 'Fault wrapper accepts trigger mode'
    ...
    Require-Text $faultPs 'J-Link|RTT|single|占用|ownership' 'Fault wrapper records J-Link ownership ordering'
