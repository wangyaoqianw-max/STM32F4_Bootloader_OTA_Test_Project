 ...
 * All Rights Reserved.
 *
 * @file s05_ymodem_flash_sink.c
 * @brief S05 Slot B YMODEM Flash Sink 实现
 * @author YaoQian Wang
 * @date 2026-09-14
 * @version V1.0
 *
...
//******************************** Private Functions ************************//
static void s05_ymodem_flash_sink_reset_runtime(s05_ymodem_flash_sink_t *sink)
{
...
static platform_error_t s05_ymodem_flash_sink_fail(
...
static platform_error_t s05_ymodem_flash_sink_prepare_payload(
    s05_ymodem_flash_sink_t *sink)
{
    ...
    if (validation != FIRMWARE_IMAGE_VALIDATION_VALID) {
        return s05_ymodem_flash_sink_fail(sink, PLATFORM_ERR_INVALID_PARAM);
    }
...
static platform_error_t s05_ymodem_flash_sink_begin(
    void *context,
    const char *filename,
    uint32_t fileSize)
{
    s05_ymodem_flash_sink_t *sink = (s05_ymodem_flash_sink_t *)context;
    ...
    s05_ymodem_flash_sink_reset_runtime(sink);
...
static platform_error_t s05_ymodem_flash_sink_consume_header(
    s05_ymodem_flash_sink_t *sink,
    const uint8_t *data,
    uint32_t *dataOffset,
    uint32_t *remaining)
{
    ...
    *remaining -= copyLength;
    ...
    if (sink->headerFillCount == FIRMWARE_IMAGE_HEADER_SIZE) {
        result = s05_ymodem_flash_sink_prepare_payload(sink);
        if (result != PLATFORM_ERR_OK) {
            return result;
        }
    }
...
static platform_error_t s05_ymodem_flash_sink_consume_payload(
    s05_ymodem_flash_sink_t *sink,
    const uint8_t *data,
    uint32_t *dataOffset,
    uint32_t *remaining)
...
static platform_error_t s05_ymodem_flash_sink_write(
    void *context,
    const uint8_t *data,
    uint32_t length)
{
    ...
    while (remaining != 0U) {
        if (sink->headerFillCount < FIRMWARE_IMAGE_HEADER_SIZE) {
            result = s05_ymodem_flash_sink_consume_header(
                sink, data, &dataOffset, &remaining);
        } else {
            result = s05_ymodem_flash_sink_consume_payload(
                sink, data, &dataOffset, &remaining);
        }
...
static platform_error_t s05_ymodem_flash_sink_end(void *context)
{
    s05_ymodem_flash_sink_t *sink = (s05_ymodem_flash_sink_t *)context;
    platform_error_t result;
    ...
    if ((sink->receivedFileBytes != sink->expectedFileSize) ||
        ...
        (sink->payloadWritten != sink->header.imageSize)) {
        return s05_ymodem_flash_sink_fail(sink, PLATFORM_ERR_INVALID_STATE);
    }
...
static void s05_ymodem_flash_sink_abort(void *context)
{
    s05_ymodem_flash_sink_t *sink = (s05_ymodem_flash_sink_t *)context;
    ...
    ymodem_sink_t *contract)
{
    ...
    contract->begin = s05_ymodem_flash_sink_begin;
    contract->write = s05_ymodem_flash_sink_write;
    contract->end = s05_ymodem_flash_sink_end;
    contract->abort = s05_ymodem_flash_sink_abort;
