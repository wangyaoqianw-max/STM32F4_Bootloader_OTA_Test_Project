...
S07A 将在不改变上述业务 ownership 和 IPC 语义的前提下，重新定义启动生命周期：`appSystem` 从长期 foreground task 改为一次性 Bootstrap/Supervisor，长期前台业务迁移到独立 `appMainTask`。S07A 完成后，以 S07A Handoff 作为最新 Application Startup Contract。
