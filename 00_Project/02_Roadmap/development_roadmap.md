...
| `S05A_Debug_Crash_Diagnostics` | 建立 Agent 可调用的 GDB 在线调试与运行态快照能力 | GDB/J-Link 配置；Runtime Snapshot resume/halt 脚本；PowerShell/BAT 入口；失败清理；J-Link 释放；真实板测 | S05；Keil AXF；J-Link GDB Server；GNU Arm GDB；STM32F411CE SWD | `READY_FOR_IMPLEMENTATION` | Resume/Halt 快照通过；`continue& → disconnect → quit` 可恢复 MCU；halt 模式保持暂停；无隐式 Flash 编程；失败路径、PID 清理和 J-Link 释放通过 |
...
### S05 - S05A - S07: OTA Download Path and Debug Gate

建立 Firmware 从 PC 进入设备到 Application 设置升级请求的完整下载链。
...
S05 已解决可靠文件运输；S05A 补齐 Agent 可调用的 GDB 调试与运行态证据；S06 解决正式并发运行模型；S07 才组合业务状态和升级请求。
...
不阻塞 S06，但必须在 `S07_OTA_Service_V1` 关闭前完成真实硬件验证。
...
Roadmap State: READY_FOR_IMPLEMENTATION
Branch: main
Baseline Commit: ec6119f64306d027c86208329823cf42b77ceebf
Scope: GDB automation and real-board verification
Next action: implement Runtime Snapshot resume/halt entrypoints
```

S05A 当前已完成 GDB 手工兼容性与控制能力板测，包括 Breakpoint、Continue、Next、Step、Backtrace、Memory Read、Variable Read，以及 `continue& → disconnect → quit` 后 MCU 恢复运行的行为确认。CmBacktrace 源码尚未下载，Fault/CmBacktrace 和 S04 Persistence 回归不属于当前 GDB checkpoint。
